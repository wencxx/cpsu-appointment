...
function ApproveApplication() {
  ...
  const approveApplication = async (applicationId) => {
    try {
      const res = await axios.put(`${endpoint()}/approve-application/${applicationId}`);
      if (res.data === 'Application approved.') {
        setApplications((prev) => prev.filter(app => app._id !== applicationId));
  ...
  const confirmApproveApplication = (applicationId) => {
  ...
  const handleApproveApplication = async () => {
    if (applicationToApprove) {
      await approveApplication(applicationToApprove);
  ...
  return (
    <>
      <div className="mb-10 w-full flex justify-between">
        <h1 className="font-medium text-lg uppercase">List of Pending Student ID Card</h1>
      </div>
                ...
                <TableCell>
                  <div className="flex justify-center gap-x-1">
                    <TooltipProvider>
                      <Tooltip>
                        <TooltipTrigger>
                          <Check className="cursor-pointer" size={17} color="green" onClick={() => confirmApproveApplication(application._id)} />
                        </TooltipTrigger>
                        <TooltipContent>
      ...
      <AlertDialog open={openApproveDialog} onOpenChange={setOpenApproveDialog}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel onClick={() => setOpenApproveDialog(false)}>Cancel</AlertDialogCancel>
            <AlertDialogAction className="bg-primary-color hover:bg-green-700" onClick={handleApproveApplication}>Approve</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
